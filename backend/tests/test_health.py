...
def test_health_endpoint_preserves_original_fields(client):
    ...
    for key in (
        "status", "service", "version", "model_loaded", "model_name",
        "device", "torch_version", "transformers_version", "uptime_seconds",
    ):
        assert key in body, f"missing original field: {key}"

    assert body["status"] == "healthy"
    assert body["model_name"] == "Wav2Vec2 Deepfake Detector"
...
def test_health_endpoint_new_additive_fields(client):
    ...
    assert set(body["models"].keys()) == {"wav2vec2_loaded", "aasist_loaded", "rawnet2_loaded"}
    assert body["models"]["wav2vec2_loaded"] is True
    assert body["models"]["aasist_loaded"] is False
